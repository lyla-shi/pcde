...
def main():
    ...
    sql_select_fixn_dur = (
        " select a.fixn_dur_avg, a.fixn_dur_sd, a.scanpath_id "
        " from sandbox.rdtm_metrics_user_doc_viewnum a "
        " where a.userid = %s "
    ...
    try:
        ...
        for userid in cur_select_userid:
            ...
            for fixn in cur_select_fixn_dur:
                list_fixn_avg.append(fixn['fixn_dur_avg'])
                list_fixn_sd.append(fixn['fixn_dur_sd'])
                list_scanid.append(fixn['scanpath_id'])

            avg_lower_whisker, avg_upper_whisker = calc_whisker(list_fixn_avg)
            sd_lower_whisker, sd_upper_whisker = calc_whisker(list_fixn_sd)
            list_fixn_dur_p = []

            for i in range(0, len(list_fixn_avg)):
                if list_fixn_avg[i] < avg_lower_whisker:
                    normalize_avg = 0
                elif list_fixn_avg[i] > avg_upper_whisker:
                    normalize_avg = 1
                else:
                    normalize_avg = (list_fixn_avg[i] - avg_lower_whisker)/(avg_upper_whisker - avg_lower_whisker)

                if list_fixn_sd[i] < sd_lower_whisker:
                    normalize_sd = 0
                elif list_fixn_sd[i] > sd_upper_whisker:
                    normalize_sd = 1
                else:
                    normalize_sd = (list_fixn_sd[i] - sd_lower_whisker)/(sd_upper_whisker - sd_lower_whisker)


                #append list
                list_fixn_dur_p.append([float(round(normalize_avg, 10)),float(round(normalize_sd, 10)), list_scanid[i]])
